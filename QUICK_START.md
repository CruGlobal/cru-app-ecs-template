...
infrastructure (the AWS ECS service, RDS if needed, secrets, and the GitHub
...
[`.github/workflows/build-deploy-ecs.yml`](./.github/workflows/build-deploy-ecs.yml).
`build.sh` builds and pushes the container image; a successful build then
triggers an ECS deploy in
